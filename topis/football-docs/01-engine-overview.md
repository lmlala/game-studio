...
**验收标准**：
- [机器] ENG-07 的架构门禁覆盖本卡边界：L2 不互引、StatePatch 必带 cause_id、
  L4 与评估旁路不参与权威模拟；
- [人工] 新增一个子引擎只需：实现 trait + 注册到管线 + 写卡片，不改其他引擎。

**评估钩子**：
- `source_engine` 覆盖率 = 100%；
- ENG-07 门禁失败按边界类型归因。
...
**验收标准**：
- [机器] ENG-07 的 tick 门禁覆盖本卡顺序：阶段越权写入、同 seed 日志序列、
  阶段 11 存读档续跑一致性；
...
状态: draft · 优先级: P0 · 依赖: ENG-01, ENG-03
...
状态: draft · 优先级: P0 · 依赖: ENG-01, ENG-02, ENG-03, ENG-04, SEED-03

**目的**：把基础架构的机器验收集中成一张门禁卡，避免 ENG-01/02 为了写清
`cargo test` 细节而膨胀。

**设计理念**：G4、G5、G8。架构卡写边界，门禁卡写如何证明边界；测试必须
可重复、可跑批、可定位失败来源。

**如何设计**：
- 依赖门禁：CI 检查 L2 引擎模块无互相 `use`；例外写入人工维护白名单；
- 写入门禁：所有 StatePatch 必带 cause_id，debug 模式缺失即失败；
- tick 门禁：同 seed 连跑 3 次，忽略时间戳等非确定字段后日志序列一致；
- 存档门禁：任意 tick 阶段 11 后存档，读档续跑与不中断运行一致；
- 回归门禁：固定 seed 集输出逐 tick 状态哈希，基线更新必须人工确认。

**验收标准**：
- [机器] `cargo test` 能一次跑完依赖、写入、tick、存档、回归五类门禁；
- [机器] 任一门禁失败时输出卡片 ID、阶段、引擎 ID、cause_id 或状态域；
- [人工] 新增门禁只能引用已有架构卡的边界，不把新机制写进本卡。

**评估钩子**：
- 门禁失败次数按类型统计；
- 回归哈希变更数与对应配置哈希一起进入跑批报告。
