...
当前实现中，角色由 `RoleFactory` 创建，已注册 `planner / proposer /
critic / referee` 四类。新增角色类型需要实现 `TemplatedRole` 子类并调用
`RoleFactory.register()`；cast.yaml 增删同类角色不改 loop 逻辑。
...
roles:
  planner:
    model: workhorse
    output: plan                 # 任务卡 → goal/todos/constraints/risks
  proposer:
    model: workhorse
...
// plan（规划者）
{ "goal": "本次运行要达成什么",
  "todos": [{"card_id": "EVT-01", "focus": "本卡本次重点"}],
  "constraints": ["边界条件"],
  "risks": ["预判风险"] }
...
{ "scores": {"维度": 1-5}, 
  "issues": [{ "id": "I-121", "severity": "blocking|major|minor",
               ...
               "suggestion": "..." }],
  "praise": ["值得保留的点"],
  "skill_requests": ["deterministic-sim"] }                    // 下一轮申请装载
...
0. 规划者读任务卡、目标卡片清单、topic 记忆，产出 plan；`plan.json`
   落盘，todo 状态由内核推进；
1. 组装各角色上下文（角色隔离视图；按预算裁剪）；
2. 并行调用批判者（当前实现串行，语义上互不可见；模拟用户为后续扩展）；
3. 门禁过滤无证据 issue；收集批判者 `skill_requests` 供下一轮裁决；
